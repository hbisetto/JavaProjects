...
> 
![Imagem de Herança 1](Heranca1.jpg)
+ Conceitos importantes de Herança:
![Imagem de Herança 2](Heranca2.jpg)
+ Tipos de Herança:

![Imagem de Herança 3](Heranca3.jpg)

1) Herança de Implementação:   
também chamado de herança pobre, pois não adiciona atributos ou métodos. Como no exemplo do visitante.

2) Herança para Diferença:  
caso dos exemplos acima de Aluno e Professor, onde ambos tem atributos e métodos específicos.

+ Abstrato e Final   
![Imagem de Herança 4](Heranca4.jpg)
...
public double somar(double t1, double t2){
    ...
}
```
Diferentes:
```java
public double somar(double v1, double v2, String v3){
    ...
}

public double somar(double t1, double t2, double t3, int t4){
    ...
}
```
+ Tipos de Polimorfismo (2 tipos mais utilizados):    
     + Sobreposição (`@Override`): acontece quando substituímos um método de uma superclasse na sua subclasse, usando a mesma assinatura. 
     Exemplo: o que acontece com o método `toString()`. **SOBREPOSIÇÃO: Mesma assinatura em classes diferentes*(@Override)*.** 


    +   Sobrecarga: o método vai variar de acordo com o parâmetro passado; assinaturas diferentes em uma mesma classe.
    
![Herança 5](Heranca5.jpg)
## Comandos em Java:
